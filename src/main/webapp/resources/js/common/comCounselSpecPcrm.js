...
//test
	...
	//hhs 부서 추가
	var orgFullNm = $("#cmscsp_cnslMainUsr").val().split(' > ');
	...
	var loParam = {
			...
			"map" : {
				...
				"callback" : callback,
				"dest_info" : orgFullNm[1]+"^"+sndPhoneNum.replace(/-/gi, ""),
//				"dest_info" : "테스트^01049364152",
				"cont_length" : parseInt(charByteSize(sendMsg)),
				"org_fullNm" : orgFullNm[0]+"^"+orgFullNm[1], 	// 부서^이름
				"org_id" : distId,		// 공무원ID
...
function minwonCnslAlrmSendSMS(reqview){
	...
	var mainDpt=["",""];
	var subDpt=["",""];
	var prefixMsgToCust="안녕하십니까? 순천시3114온누리콜센터입니다. 접수내용이  ";
	var suffixMsgToCust="로 이관됩니다. 전화주셔서 감사합니다. 편안한 하루 보내세요.";
	var prefixMsgToUsr="온누리콜센터로부터 민원이 이관 됐습니다. 확인부탁드립니다.";
	var sendMsg=["","",""];
	var sendNum=["","",""];
	var deptDist=["","",""];
	var deptId=["","",""];
...
function btnUpdate_clickEvent()
{
	...
	if(cmscsp_popupAddr != cmscsp_checkAddr){
		$.ajax({
			type : "post",
			dataType: "json",
			async : true,
...
function getJsonStrcmscspUpdateAddrInfo(){
	var loParam = {
			...
			"map" : {
				...
				"spch_type" : "9",							//결과코드 -> 상담원이 바꿨을 경우
				"stt_rcg_rst" : $("#cmscsp_selMainAddrCd").val(),	//음성인식 결과
			}
		};
		...
		$.ajax({
			...
			{	
				if(g_changedCont == ""){  //수정사항X
					if(actType != "010000" && actType != "030100" && actType != "030400"){ //여기1
						$.ajax({
							type : "post",
							dataType: "json",
							async : true,
					...
					else if(actType != "030100" && oldActType == "030100"){ //여기2
						$.ajax({
							type : "post",
							dataType: "json",
							async : true,
					...
					$.ajax({
						...
						success : function(data)
						{
							if(actType != "010000" && actType != "030100" && actType != "030400"){
							$.ajax({
								type : "post",
								dataType: "json",
								async : true,
							...
							}else if(actType != "030100" && oldActType == "030100"){
								$.ajax({
									type : "post",
									dataType: "json",
									async : true,
...
//	setSelectBoxWithCode("cmscsp_callbckActStCd", "", "90020", "CHILD", "", "");	// 처리유형 셋팅	
	setSelectBoxWithCode("cmscsp_callbckActStCd", "", "90020", "PCRM2", "", "");	// 처리유형 셋팅	
...
{	
	...
	$.ajax({
		...
		{
			console.log(data);
			$("#cmscsp_selMainAddrCd").val(data.STT_RCG_RST);
			cmscsp_popupAddr = data.STT_RCG_RST;
			cmscsp_spchType = data.SPCH_TYPE;
			
			if(cmscsp_spchType == "8" || cmscsp_spchType == "9"){ //상담사가 직접 저장했을때
				$("#cmscsp_selMainAddrCd").prop("disabled",false);
			}else if(cmscsp_spchType == "0"){
				$("#cmscsp_selMainAddrCd").prop("disabled",true);
//				$("#cmscsp_selMainAddrDetail").html("");
			}else if(cmscsp_spchType == "3"){
//				$("#cmscsp_selMainAddrDetail").html(data.H_ADDR);
			}else if(cmscsp_spchType == "5"){
...
//				$("#cmscsp_modDt").html("&nbsp; "+data.MOD_DT_FORMAT + " " + data.MOD_TM_FORMAT+ "<br>&nbsp; "+ data.MOD_USR_NM); // 최종수정
				$("#cmscsp_modDt").html("&nbsp; "+data.MOD_DT_FORMAT + " " + data.MOD_TM_FORMAT); // 최종수정
