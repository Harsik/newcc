		...
		$.ajax({
			...
			success : function(data)
			{
				...
				var InOut=0;
					...
					$("#idx_agentLogin").html("0"); // 상담 로그인
					$("#idx_agentReady").html("0"); // 상담 대기
					$("#idx_agentBusy").html("0"); // 상담 통화
					$("#idx_afterWork").html("0"); // 상담 후처리
					$("#idx_custWait").html("0"); //고객 대기호
					$("#idx_totalCall").html("0"); // 인입호
					$("#idx_serviceLevel").html("0"); //서비스레벨
					$("#idx_inbound").html("0"); // IB
					$("#idx_abandon").html("0"); // 포기호
					$("#idx_answerRate").html("0"); // 응답율
					$("#idx_outbound").html("0"); // OB
					$("#idx_IbObCall").html("0"); //총처리호  IB+OB
				// console.log(data);
					$.each(data, function(key, state)
					{
					   if(state.TYPE=="AGENTSTATE") {
						   // WAIT_COUNT
						   // TOTAL_COUNT
							$("#idx_agentLogin").html(state.LOGON_COUNT); // 상담 로그인
							$("#idx_agentReady").html(state.READY_COUNT); // 상담 대기
							$("#idx_agentBusy").html(state.BUSY_COUNT); // 상담 통화
							$("#idx_afterWork").html(state.ACW_COUNT); // 상담 후처리
						}else if(state.TYPE=="CALLSTATE") {
							$("#idx_custWait").html(state.WAIT_COUNT); //고객 대기호
							$("#idx_totalCall").html(numberWithCommas(state.TOTAL_COUNT)); // 인입호
							$("#idx_serviceLevel").html(state.LOGON_COUNT); //서비스레벨
							$("#idx_inbound").html(numberWithCommas(state.READY_COUNT)); // IB
							InOut += state.READY_COUNT;
							$("#idx_abandon").html(state.BUSY_COUNT); // 포기호
							$("#idx_answerRate").html(state.ACW_COUNT); // 응답율
						}else if(state.TYPE=="OUTBOUND"){
							InOut +=state.WAIT_COUNT;
							$("#idx_outbound").html(numberWithCommas(state.WAIT_COUNT)); // OB
						}

					});

					$("#idx_IbObCall").html(numberWithCommas(InOut)); //총처리호  IB+OB
		...
		$.ajax({
			...
			success : function(data)
			{
				if(data != null)
				{
					...
					$("#idx_cnslCount01").html(data_etc +"건");
					$("#idx_cnslCount02").html(data.GREENPEACE+"건");
					$("#idx_cnslCount03").html(data.CULTURE+"건");
					$("#idx_cnslCount04").html(data.WELFARE+"건");
					$("#idx_cnslCount05").html(data.CITY+"건");
					$("#idx_cnslCount06").html(data.TRAFFIC+"건");
					$("#idx_cnslCount07").html(data.GE_ADMINIST+"건");
					$("#idx_cnslCount08").html(data.AGRIFISH+"건");
					
					$("#idx_cnslCountt02").html(numberWithCommas(data.TOTALCOUNT)+"건");
					$("#idx_cnslCountt03").html(numberWithCommas(data.TOTALCOUNT)+"건");
		 ...
		 g_DashBoardArray01['농수축산']=data.AGRIFISH;
					...
					$("#reqCount03").html(data.JEJU_ETC+"건");
		 			...
		 			$("#idx_administCount01").html(data.JEJU_DO +"건");
					$("#idx_administCount02").html(data.JEJU_CI+"건");
					$("#idx_administCount03").html(data.JEJU_SE+"건");
					$("#idx_administCount04").html(data.JEJU_ET+"건");
				...
				{
					$("#idx_administCount01").html("0건");
					$("#idx_administCount02").html("0건");
					$("#idx_administCount03").html("0건");
					$("#idx_administCount04").html("0건");

					$("#idx_cnslCount01").html("0건");
					$("#idx_cnslCount02").html("0건");
					$("#idx_cnslCount03").html("0건");
					$("#idx_cnslCount04").html("0건");
					$("#idx_cnslCount05").html("0건");
					$("#idx_cnslCount06").html("0건");
					$("#idx_cnslCount07").html("0건");
				}
			},
			error : function(data, status, err)
			{
...
function startTime() {
    ...
    m = checkTime(m);
    s = checkTime(s);
    $("#idx_curDate1").html(year + ". " + month + ". " + date +"("+day+") &nbsp; ");
    $("#idx_curTime1").html(h + ":" + m + ":" + s);
    $("#idx_curDate2").html(year + ". " + month + ". " + date +"("+day+") &nbsp; ");
    $("#idx_curTime2").html(h + ":" + m + ":" + s);
    $("#idx_curDate3").html(year + ". " + month + ". " + date +"("+day+") &nbsp; ");
    $("#idx_curTime3").html(h + ":" + m + ":" + s);
...
function drawChart() {
	...
	//#1c0000
	var chart1 = new google.visualization.ColumnChart(document.getElementById('idx_chart_div'));
...
// 차트 데이터 설정
	...
	var chart2 = new google.visualization.PieChart(document.getElementById('idx_pie_div2'));
